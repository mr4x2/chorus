...
func FromConfigAndEnv(c *struct {
    ...
    // overlay env
    env := FromEnv()
    if env.DSN != "" { cfg.DSN = env.DSN }
    if v := os.Getenv("DB_HOST"); v != "" { cfg.Host = env.Host }
    if os.Getenv("DB_PORT") != "" { cfg.Port = env.Port }
    if v := os.Getenv("DB_USER"); v != "" { cfg.User = env.User }
    if v := os.Getenv("DB_PASSWORD"); v != "" { cfg.Password = env.Password }
    if v := os.Getenv("DB_NAME"); v != "" { cfg.DBName = env.DBName }
    if v := os.Getenv("DB_SSLMODE"); v != "" { cfg.SSLMode = env.SSLMode }
    if os.Getenv("DB_MAX_OPEN_CONNS") != "" { cfg.MaxOpenConns = env.MaxOpenConns }
    if os.Getenv("DB_MAX_IDLE_CONNS") != "" { cfg.MaxIdleConns = env.MaxIdleConns }
    if os.Getenv("DB_CONN_MAX_LIFETIME") != "" { cfg.ConnMaxLifetime = env.ConnMaxLifetime }
    if os.Getenv("DB_CONN_MAX_IDLE_TIME") != "" { cfg.ConnMaxIdleTime = env.ConnMaxIdleTime }
    if v := os.Getenv("DB_LOG_LEVEL"); v != "" { cfg.LogLevel = env.LogLevel }
    return cfg
}
