...
CREATE TABLE IF NOT EXISTS replicate_job (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT fk_replicate_job_from_storage FOREIGN KEY (from_id) REFERENCES storage(id) ON DELETE CASCADE,
    CONSTRAINT fk_replicate_job_to_storage   FOREIGN KEY (to_id)   REFERENCES storage(id) ON DELETE CASCADE
);
...
DECLARE
    ...
    main_storage_id   UUID := uuid_generate_v4();
    follower_storage_id UUID := uuid_generate_v4();
BEGIN
    -- Insert storages if names don't exist
    IF NOT EXISTS (SELECT 1 FROM storage WHERE name = 'main-storage') THEN
        INSERT INTO storage (id, name, address, provider, is_main, is_secure, default_region, health_check_interval_ms, http_timeout_ms,
                             rate_limit_enabled, rate_limit_rpm, project_id, access_key_id, secret_access_key)
        VALUES (main_storage_id, 'main-storage', 'http://localhost:9000', 'minio', true, false, 'us-east-1', 0, 0,
                false, 0, sample_project_id, 'minioadmin', 'minioadmin');
    ELSE
        SELECT id, project_id INTO main_storage_id, sample_project_id FROM storage WHERE name = 'main-storage' LIMIT 1;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM storage WHERE name = 'follower-storage') THEN
        INSERT INTO storage (id, name, address, provider, is_main, is_secure, default_region, health_check_interval_ms, http_timeout_ms,
                             rate_limit_enabled, rate_limit_rpm, project_id, access_key_id, secret_access_key)
        VALUES (follower_storage_id, 'follower-storage', 'http://localhost:9001', 'minio', false, false, 'us-east-1', 0, 0,
                false, 0, sample_project_id, 'minioadmin', 'minioadmin');
    ELSE
        SELECT id INTO follower_storage_id FROM storage WHERE name = 'follower-storage' LIMIT 1;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        VALUES (uuid_generate_v4(), sample_project_id, 'test-bucket', main_storage_id, follower_storage_id, 'test-bucket-copy', 'pending');
    END IF;
END $$;
...
SELECT id, name, address, provider, is_main, project_id FROM storage ORDER BY name;

SELECT 'Replicate job records (joined):' AS info;
SELECT rj.id, rj.project_id, rj.bucket, rj.status,
       fs.name AS from_storage, ts.name AS to_storage, rj.to_bucket
